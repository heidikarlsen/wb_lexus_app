...
if not IR_Network_relevance.empty:
    ...
    ranked_IR_Network = IR_Network_relevance[
        IR_Network_relevance["relevance_score"] >= minimum_score
    ].copy()
    ...
    col1.metric(
        "IR_Network above selected score",
        len(ranked_IR_Network)
    )

    col2.metric(
        "Highly relevant IR_Network",
        int(
            (
                IR_Network_relevance["relevance_score"]
                >= high_relevance_threshold
            ).sum()
        )
    )
    ...
    col3.metric(
        "Total IR_Network analysed",
        len(IR_Network_relevance)
    )
    ...
    st.markdown("#### IR_Network ranked by relevance")
    ...
    st.dataframe(
        ranked_IR_Network[display_columns],
        use_container_width=True,
        hide_index=True
    )
    ...
    time_df = IR_Network_relevance.dropna(subset=["year"]).copy()
    ...
    fig_average = px.line(
        ...
        title="Average IR_Network relevance by year"
    )
    ...
    # 2. Number of highly relevant IR_Network per year
    relevant_by_year = (
        time_df
        .groupby("year")["highly_relevant"]
        .sum()
        .reset_index(name="highly_relevant_IR_Network")
    )

    fig_number = px.bar(
        relevant_by_year,
        x="year",
        y="highly_relevant_IR_Network",
        labels={
            "year": "Year",
            "highly_relevant_IR_Network": "Highly relevant IR_Network"
        },
        title="Number of highly relevant IR_Network by year"
    )
    ...
    # 3. Share of IR_Network that are highly relevant
    share_by_year = (
        time_df
        .groupby("year")
        .agg(
            total_IR_Network=("filename", "count"),
            highly_relevant_IR_Network=("highly_relevant", "sum")
        )
        .reset_index()
    )

    share_by_year["share_relevant"] = (
        share_by_year["highly_relevant_IR_Network"]
        / share_by_year["total_IR_Network"]
        * 100
    )
    ...
    fig_share = px.line(
        ...
        labels={
            "year": "Year",
            "share_relevant": "Highly relevant IR_Network (%)"
        },
        title="Share of highly relevant IR_Network by year"
    )
